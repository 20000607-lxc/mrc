...
class MRCNERDataset(Dataset):
    ...
    def __init__(self, json_path, tokenizer: BertWordPieceTokenizer, max_length: int = 128, possible_only=False,
                 is_chinese=False, pad_to_maxlen=False, task='train', percent=10):
        ...
        if task == 'train':
            all_random_samples = random.sample(range(0, len(self.all_data)), math.ceil(len(self.all_data)*int(percent)/100))
        else:
            all_random_samples = random.sample(range(0, len(self.all_data)), len(self.all_data))

        self.all_data = [
            self.all_data[x] for x in range(len(self.all_data)) if x in all_random_samples
        ]
