...
class BertLabeling(pl.LightningModule):
    ...
    def __init__(
        ...
        if isinstance(args, argparse.Namespace):
            ...
            self.args  = args['args']#TmpArgs(**args)
            args = args['args']
    ...
    def validation_epoch_end(self, outputs):
        ...
        wandb.log({'val_loss': avg_loss, 'log': tensorboard_logs})
    ...
    def get_dataloader(self, prefix="train", percent=10, limit: int = None) -> DataLoader:
        ...
        # todo vocab难道不能用bert-large-cased? 更换tokenizer时需要wget vocab file
        dataset = MRCNERDataset(json_path=json_path,
                                # tokenizer=BertTokenizer.from_pretrained('bert-base-cased'),
                                tokenizer=BertWordPieceTokenizer(vocab_path),
                                max_length=self.args.max_length,
                                is_chinese=self.chinese,
                                pad_to_maxlen=False, task=prefix, percent=percent
                                )
...
def main():
    ...
    if args.pretrained_checkpoint:
        model.load_state_dict(torch.load(args.pretrained_checkpoint,
                                         map_location=torch.device('gpu'))["state_dict"])
    checkpoint_callback = ModelCheckpoint(
        filepath="saved_models/bert/",#args.default_root_dir,
        save_top_k=1,
        monitor="span_f1",
        period=-1,
        prefix="bert_",
